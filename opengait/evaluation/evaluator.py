...
def evaluate_indoor_dataset(data, dataset, metric='euc', cross_view_gallery=False):
    ...
    if dataset == 'CASIA-B':
        # np.save('./Baseline_feature.npy', feature, allow_pickle=True)
        # np.save('./Baseline_label.npy', label, allow_pickle=True)
        # np.save('./Baseline_seq.npy', np.array(seq_type), allow_pickle=True)
        np.save('./GaitGS_feature.npy', feature, allow_pickle=True)
        np.save('./GaitGS_label.npy', label, allow_pickle=True)
        np.save('./GaitGS_seq.npy', np.array(seq_type), allow_pickle=True)
